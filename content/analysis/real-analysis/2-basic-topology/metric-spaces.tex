...
\begin{proof}
    ...
    \begin{align*}
        ...
        &< \rho\left( x_{1},y \right) + \epsilon_{1} - \rho \left( x_{1},y \right) \comment{Since $z \in B_{\rho} \left( y, \epsilon \right)$}\\
       &< \epsilon_{1}
    \end{align*}
...
\begin{lemma}
    Let \( \mathbf{V} \) be a normed space. Then \( \mathbf{V} \) has a \vocab{metric induced by the norm} defined by 
...
\begin{exercise}
    Show that in a normed space \( \abs{ \; \norm{\vb{x}} - \norm{\vb{y}}\; } \le \norm{\vb{x} - \vb{y}}. \)
\end{exercise}
\begin{solution}
    ...
    \end{align*}
    Applying the absolute value to both sides resolves this exercise.
\end{solution}