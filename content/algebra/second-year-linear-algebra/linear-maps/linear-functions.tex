...
\begin{theorem}[Rank-Nullity]\label{thm:Rank-Nullity}
    Suppose that \( T \in \mathcal{L} \left( \mathbf{V}, \mathbf{W} \right) \), then 
    \[ \mathrm{dim} \left( \mathrm{null} \left( T \right) \right) + \mathrm{dim} \left( \mathrm{range} \left( T \right) \right) = \mathrm{dim} \left( \mathbf{V} \right)  \]
\end{theorem}
\begin{proof}
    
\end{proof}

\begin{corollary}\label{thm:equal-dim-imp-equiv-of-inj-and-surj}
    If \( T \in \mathcal{L} \left( \mathbf{V}, \mathbf{W} \right) \) and \( \mathrm{dim} \left( \mathbf{V} \right) = \mathrm{dim} \left( \mathbf{W} \right) \). Then \( T \) is one-to-one if and only if it is onto. 
\end{corollary}
\begin{proof}
    Suppose that 
\end{proof}
